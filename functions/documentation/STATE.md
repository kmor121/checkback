...
-   **Next:** P0バグの詳細検査→最小差分修正→Verify全通過。
...
| `components/viewer/ViewerCanvas.jsx` | 描画キャンバス・座標変換・shape管理        |
| `components/DebugOverlay`            | エラー捕捉・表示                          |
| `components/viewer/FloatingToolbar`  | ペイント ON/OFF・ツール選択               |
...
3. **【必須】VERIFY V-01 ～ V-05 全テスト実施:**
   -   手操作テスト + コンソールログ確認。
   -   結果を VERIFY.md の「実行記録」に追記。

4. **【リスク】推奨テスト V-A01 ～ A05 の準備:**
   -   テストデータ（複数ページPDF・大量コメント・権限設定）の用意。

5. **【運用】BUGS/VERIFY 継続メンテナンス:**
