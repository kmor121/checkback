...
## B-0004: 新規コメント描画の1本目がCanvasに表示されない（V-06）

**優先度:** P0  
**影響範囲:** ViewerCanvas (renderedShapes)  
**発生頻度:** 常に（新規コメント作成時、renderTargetCommentId=null）

**再現手順:**
1. ShareView を開く。
2. 新規コメント作成モードにする（ペイントON）。
3. rect/penで1本目を描画する。
4. 描画が表示されない。

**現象:**
- shapeはshapesMapRefに正常に追加されるが、renderedShapesフィルタで除外され画面に出ない。

**期待結果:**
- 描いた位置に描画が表示されて残る。

**関連状態:**
- renderTargetCommentId: null（新規コメント時）
- draftCommentId: temp_XXXX（ShareViewのpaintContextId経由）
- showDraftOnly: false / showAllPaint: false

**原因（確定）:**
- renderedShapesのuseMemoが `renderTargetCommentId=null` の時に `return []` していた。
- 新規コメント作成時は `draftCommentId`（=paintContextId=temp_XXX）にフォールバックする必要があった。

---

**Files inspected:**
- components/viewer/ViewerCanvas.jsx (line 193-206)
- pages/ShareView.jsx (line 700-703, 3431)

**Relevant code:**
```
const targetId = normalizeNullableId(renderTargetCommentId);
if (targetId) return src.filter(s => resolveCommentId(s) === targetId);
return []; // ← ここで新規描画が除外されていた
```

---

**Fix方針:**
- targetId決定時に `draftCommentId` をフォールバックとして使用。

**Patch:**
- `const targetId = targetId0 ?? draftId;` (draftId = normalizeNullableId(draftCommentId))

---

**Verify結果:**
- ? V-06（初回描画安定性）- Verify待ち
- ? V-01（既存コメント選択）- 回帰確認待ち

**Risk / Next:**
- リスク: 既存コメント選択時のrenderTargetCommentIdが非nullなので影響なし。draftCommentIdがnullの場合も従来通り[]。
- 次: V-06 OK確認後、V-01〜V-05 回帰テスト。
